#!/usr/bin/env python3
# Adapted from https://github.com/emmi-lili/emmi-lili (scripts/banner/generate.py).
...
ROWS = [
    ...
    ("Origin", "Greater Noida, India"),
    ("Education", "GEC Khagaria · B.Tech CSE (IoT)"),
    ("Status", "Building + Learning + Shipping"),
    ("ToolChain", "VS Code · Cursor · Git"),
    ("Core.Lang", "Python · C++ · JavaScript"),
    ("Core.Frontend", "React · HTML · CSS · Tailwind"),
    ("Core.Backend", "Flask · Node · Firebase"),
    ("Core.Database", "Postgres · Firebase"),
    ("Core.Infra", "Azure · Netlify · Docker"),
    ("Core.IoT", "ESP32 · Arduino · Raspberry Pi"),
    ("Grid.Mail", "ajit.info999@gmail.com"),
    ("Grid.LinkedIn", "/in/ajit7900"),
    ("Grid.GitHub", "ajit421"),
    ("Grid.X", "@blackgold_79"),
]
...
def make_logos() -> dict[str, Image.Image]:
    ...
    # Microchip / MCU silhouette: pinned package with a die core.
    chip = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    d = ImageDraw.Draw(chip)
    pin_w, pin_len = 20, 42
    for slot in range(4):
        base = 140 + slot * 35
        d.rectangle((base, 118 - pin_len, base + pin_w, 130), fill="black")
        d.rectangle((base, 270, base + pin_w, 282 + pin_len), fill="black")
        d.rectangle((118 - pin_len, base, 130, base + pin_w), fill="black")
        d.rectangle((270, base, 282 + pin_len, base + pin_w), fill="black")
    d.rounded_rectangle((118, 118, 282, 282), radius=16, fill="black")
    d.rounded_rectangle((152, 152, 248, 248), radius=8, fill=(0, 0, 0, 0))
    d.rounded_rectangle((180, 180, 220, 220), radius=4, fill="black")
    logos["chip"] = chip
...
def portrait_points(theme: str, rng: np.random.Generator) -> np.ndarray:
    ...
    crop = source.crop((14, 10, 416, 465)).resize((300, 340), Image.Resampling.LANCZOS)
...
def render_svg(
    ...
    chip = transport(code, logo_points["chip"][:n])
    ...
    frames = [source, source, rust, rust, code, code, chip, chip, source]
    ...
    parts: list[str] = [
        ...
        "<title id=\"title\">Ajit's live system profile</title>",
        '<desc id="desc">Animated terminal profile with a dithered portrait and '
        "gear, code, and microchip silhouettes.</desc>",
        "<defs>",
